...
class ImportedBookRow(val file: File, val bookDataProvider: Option[BookDataProvider], formatResolver: BookFormatResolver) {
  ...
  val creationDate = StringProperty(bookDataProvider.flatMap(_.bookMetadata.creationDate).map(_.toString).getOrElse(""))
  val publicationDate = StringProperty(bookDataProvider.flatMap(_.bookMetadata.publicationDate).map(_.toString).getOrElse(""))
